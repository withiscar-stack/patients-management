...
def extract_text_from_pdf(uploaded_file):
    ...
    이 함수는 PyMuPDF(fitz)를 사용하여 PDF 스트림에서 텍스트를 추출합니다.
    Args:
        uploaded_file: Streamlit file_uploader로 업로드된 파일 객체.
                       이 객체는 .read() 메서드를 통해 bytes 데이터를 반환합니다.
    Returns:
    ...
    """
    try:
        # uploaded_file.read()는 bytes를 반환하므로, io.BytesIO를 사용해 fitz.open에 전달합니다.
        # fitz.open은 파일 경로 외에 bytes 스트림을 직접 받을 수 있습니다.
        doc = fitz.open(stream=io.BytesIO(uploaded_file.read()), filetype="pdf")
        text = ""
        for page_num in range(doc.page_count):
            page = doc.load_page(page_num)
            text += page.get_text() # 페이지의 모든 텍스트를 추출
        doc.close() # 문서 객체 닫기
        return text
    except fitz.FileDataError:
        # PyMuPDF에서 PDF 파일 형식이 유효하지 않을 때 발생하는 오류
        raise fitz.FileDataError("업로드된 파일이 유효한 PDF 파일이 아닙니다.")
    except Exception as e:
        # 기타 PDF 처리 중 발생할 수 있는 모든 오류를 포괄적으로 처리
...
if uploaded_file is not None:
    try:
        ...
        st.error(e)
        # 오류 발생 시 세션 상태 초기화
        if 'full_pdf_text' in st.session_state:
...
if db and 'extracted_df' in st.session_state and not st.session_state.extracted_df.empty:
    if st.button("✅ 추출된 데이터 Firestore에 저장", key="save_to_firestore_btn"):
        ...
        with st.spinner("Firestore에 데이터를 저장 중..."):
            ...
            st.success(f"Firestore에 총 {saved_count} 건의 새로운 데이터가 저장되었고, {skipped_count} 건의 기존 데이터는 건너뛰었습니다.")
...
if db:
    if st.button("📊 Firestore 데이터 불러오기", key="load_from_firestore_btn"):
        collection_name = "daily_settlements"
        try:
            with st.spinner("Firestore에서 데이터를 불러오는 중..."):
                ...
                for doc in docs:
                    ...
                    if isinstance(d.get("extraction_date"), datetime.datetime):
                        ...
                        d["extraction_date"] = "N/A" # 타임스탬프가 없는 경우 처리
